CREATE TABLE EMPLOYEE(SSN VARCHAR(10) PRIMARY KEY,FNAME VARCHAR(15),LNAME VARCHAR(15),ADDRESS VARCHAR(15),GENDER CHAR(1),SALARY INTEGER,SUPERSSN VARCHAR(10),DNO INTEGER);
CREATE TABLE DEPARTMENT(DNO INTEGER PRIMARY KEY,DNAME VARCHAR(15),MGRSSN VARCHAR(10),MGRSTARTDATE DATE,FOREIGN KEY(MGRSSN) references EMPLOYEE(SSN) ON DELETE CASCADE); 
CREATE TABLE DLOCATION(DNO INTEGER,DLOC VARCHAR(15),FOREIGN KEY(DNO) references DEPARTMENT(DNO) ON DELETE CASCADE);
CREATE TABLE PROJECT(PNO INTEGER PRIMARY KEY,PNAME VARCHAR(15),PLOCATION VARCHAR(15),DNO INTEGER,FOREIGN KEY(DNO) references DEPARTMENT(DNO) ON DELETE CASCADE);
CREATE TABLE WORKS_ON(SSN VARCHAR(10),PNO INTEGER,HOURS INTEGER,FOREIGN KEY(SSN) references EMPLOYEE(SSN) ON DELETE CASCADE,FOREIGN KEY(PNO) references PROJECT(PNO) ON DELETE CASCADE);
